...
export async function sendPortfolioContact(env, payload) {
    ...
    if (!user || !pass || !to) {
        // Fallback to local dev mock if running locally or in development mode
        if (process.env.NODE_ENV === "development" || env.NODE_ENV === "development" || !process.env.VERCEL) {
            console.warn("\n⚠️ [Mail Not Configured]: Gmail credentials missing from your .env file.");
            console.log("📩 Form Data (Simulated Delivery):", payload);
            console.log("👉 To enable real email delivery, create a .env file in the root directory and add GMAIL_USER and GMAIL_APP_PASSWORD.\n");
